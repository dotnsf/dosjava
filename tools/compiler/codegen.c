...
int generate_method(CodeGenerator* codegen, ASTNode* method_node) {
    ...
    printf("DEBUG: Code size before implicit RETURN: %u\n", codegen->context->code->size);
    if (return_type.kind == TYPE_VOID) {
        emit_opcode(codegen, OP_RETURN);
        printf("DEBUG: Added implicit RETURN at offset %u\n", codegen->context->code->size - 1);
    } else {
        printf("DEBUG: No implicit RETURN (not void method)\n");
    }
    printf("DEBUG: Final code size: %u\n", codegen->context->code->size);
    
    /* Dump generated bytecode */
    {
        uint16_t i;
        printf("DEBUG: Generated bytecode for '%s': ", method_name);
    ...
    printf("DEBUG: code_start=%u, bytecode->size=%u\n", code_start, codegen->bytecode->size);
    fflush(stdout);
    ...
    printf("DEBUG: Copied %u bytes to offset %u, new bytecode->size=%u\n",
           code_length, code_start, codegen->bytecode->size);
    fflush(stdout);
    ...
    /* Add or update method in method table */
    {
        ...
        for (method_idx = 0; method_idx < codegen->method_count; method_idx++) {
            if (codegen->methods[method_idx].name_index == name_idx) {
                ...
                printf("DEBUG: Updated method[%u] '%s': code_offset=%u, code_length=%u\n",
                       method_idx, method_name, code_start, code_length);
                fflush(stdout);
                found = 1;
                break;
            }
        ...
        if (!found && codegen->method_count < 64) {
            ...
            codegen->methods[codegen->method_count].flags =
                (is_static ? METHOD_STATIC : 0) |
                (is_public ? METHOD_PUBLIC : 0);
            printf("DEBUG: Added method[%u] '%s': code_offset=%u, code_length=%u\n",
                   codegen->method_count, method_name, code_start, code_length);
            fflush(stdout);
...
int generate_var_decl(CodeGenerator* codegen, ASTNode* var_node) {
    ...
    printf("DEBUG: generate_var_decl: var='%s', init_expr_idx=%u\n", var_name, init_expr_idx);
    fflush(stdout);
    
    /* Get local index */
    printf("DEBUG: About to call get_local_index for '%s'\n", var_name);
    fflush(stdout);
    local_idx = get_local_index(codegen, var_name);
    printf("DEBUG: get_local_index returned %u\n", local_idx);
    fflush(stdout);
    ...
    if (init_expr_idx != 0) {
        ...
        if (init_expr) {
            printf("DEBUG: Generating initializer expression\n");
...
int generate_expression(CodeGenerator* codegen, ASTNode* expr_node) {
    ...
    printf("DEBUG: generate_expression: node type = %d\n", expr_node->type);
...
uint16_t get_local_index(CodeGenerator* codegen, const char* name) {
    ...
    if (!codegen || !name) {
        printf("DEBUG: get_local_index: NULL codegen or name\n");
        return 0xFFFF;
    }
    ...
    if (!sym) {
        printf("DEBUG: get_local_index: Symbol '%s' not found in symbol table\n", name);
        return 0xFFFF;
    }
    ...
    if (sym->kind == SYM_PARAM) {
        printf("DEBUG: get_local_index: Parameter index=%u\n", sym->data.param_data.index);
        ...
        if (codegen->current_method) {
            ...
            printf("DEBUG: get_local_index: Local index=%u + param_count=%u = %u\n",
                   sym->data.local_data.index, param_count, param_count + sym->data.local_data.index);
    ...
    printf("DEBUG: get_local_index: Symbol kind %d not handled\n", sym->kind);
    return 0xFFFF;
}
