...
int generate_method(CodeGenerator* codegen, ASTNode* method_node) {
    ...
    if (method_node->data.method.return_type.kind == TYPE_VOID) {
    ...
    if (codegen->method_count < 64) {
        codegen->methods[codegen->method_count].name_index = find_or_add_utf8(codegen, method_name);
        ...
        codegen->methods[codegen->method_count].flags = 
            (method_node->data.method.is_static ? METHOD_STATIC : 0) |
            (method_node->data.method.is_public ? METHOD_PUBLIC : 0);
...
int generate_var_decl(CodeGenerator* codegen, ASTNode* var_node) {
    ...
    if (var_node->data.var_decl.init_expr != 0) {
        init_expr = codegen_get_node(codegen, var_node->data.var_decl.init_expr);
        if (init_expr) {
...
int write_djc_file(CodeGenerator* codegen) {
    ...
    if (codegen->method_count > 0) {
        if (fwrite(codegen->methods, sizeof(DJCMethod), codegen->method_count, codegen->output_file) != codegen->method_count) {
            return -1;
        }
    }
    ...
    if (codegen->field_count > 0) {
        if (fwrite(codegen->fields, sizeof(DJCField), codegen->field_count, codegen->output_file) != codegen->field_count) {
            return -1;
        }
    }
