...
typedef enum {
    ...
    NODE_POSTFIX_OP,        /* Postfix operation (++, --) */
...
/* Binary operators - AST level (different from VM opcodes) */
typedef enum {
    BINOP_ADD,         /* + */
    BINOP_SUB,         /* - */
    BINOP_MUL,         /* * */
...
/* Postfix operators - AST level */
typedef enum {
    POSTOP_INC,      /* ++ (increment) */
    POSTOP_DEC       /* -- (decrement) */
} PostfixOp;
...
typedef struct ASTNode {
    ...
    union {
        ...
        /* Binary operation */
        struct {
            uint16_t op;            /* BinaryOp as uint16_t for alignment */
        ...
        /* Unary operation */
        struct {
            uint16_t op;            /* UnaryOp as uint16_t for alignment */
            uint16_t operand;       /* Index of operand */
        } unary_op;
        
        /* Postfix operation */
        struct {
            uint16_t op;            /* PostfixOp as uint16_t for alignment */
            uint16_t operand;       /* Index of operand */
        } postfix_op;
